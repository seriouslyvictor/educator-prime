...
> - Phase 7 env/docs complete: updated API and web `.env.example` files with Sentry, admin, retention, payload, and deployment notes. Gate: `uv run --extra dev pytest -q` in `apps/api` -> 178 passed, 4 skipped; `pnpm build` in `apps/web` -> build succeeded. Remaining Phase 7 action: run `graphify update .` and commit its output separately.
