...
## Corpus Check
- 66 files · ~44,505 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1017 nodes · 4203 edges · 65 communities (55 shown, 10 thin omitted)
- Extraction: 59% EXTRACTED · 41% INFERRED · 0% AMBIGUOUS · INFERRED: 1721 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3867a84c`
...
1. `log_event()` - 89 edges
2. `get_settings()` - 81 edges
3. `GradingStatus` - 76 edges
4. `GradingSubmission` - 72 edges
5. `GoogleProvider` - 71 edges
6. `GradingJob` - 71 edges
7. `Session` - 70 edges
8. `GradingEngine` - 59 edges
9. `GradingFileCache` - 57 edges
10. `ExportStatus` - 56 edges
...
Cohesion: 0.10
Nodes (97): GradingFileCache, datetime, ExtractedSubmissionContent, GoogleProvider, GradingEngine, GradingFileCache, GradingJob, GradingJobRead (+89 more)

### Community 1 - "API Routes & Job Management"
Cohesion: 0.37
Nodes (72): Activity, datetime, GoogleProvider, GradingEngine, GradingJob, GradingJobRead, Path, PrivacyAuditRead (+64 more)

### Community 2 - "LLM Grading Engine"
Cohesion: 0.20
Nodes (16): Any, LlmModelEntry, GradingEngineResult, _bounded_number(), _build_rubric_messages(), build_sample_xml(), _get_field(), parse_rubric_criteria() (+8 more)

### Community 3 - "Privacy Audit System"
Cohesion: 0.08
Nodes (55): Session, _criteria_match_defaults(), delete_job_cache(), grading_job_snapshot(), Classroom Downloader API., _as_utc(), auth_callback(), auth_logout() (+47 more)

### Community 4 - "Frontend Navigation UI"
Cohesion: 0.08
Nodes (21): DryRunDrawer(), ProgressLogItem, ProgressView(), api, ensureDirectory(), exportJobToFolder(), isFolderExportSupported(), pickExportFolder() (+13 more)

### Community 5 - "Test Fixtures & Settings"
Cohesion: 0.07
Nodes (45): get_settings(), Multi-file Submission Grouping, _CapturingEngine (grading test double), _enable_litellm_engine helper, _seed_infer_job helper, _seed_preview_cache helper, _sse_payloads(), test_brief_mode_sends_rubric_text_and_keeps_default_criteria() (+37 more)
...
Cohesion: 0.07
Nodes (33): Safe Source Label (no PII in filenames), Privacy Scrub Pipeline (pt-BR), Student Pseudonymization before AI, Teacher-Loop Modes (off/approve/auto/cowrite), AuditStrip / AuditReport, BlockedEvidence, BlockedEvidence(), extensionOf() (+25 more)

### Community 9 - "Frontend API & Caching"
Cohesion: 0.13
Nodes (16): Stale-While-Revalidate API Cache Pattern, AuditStat, PreparedPanel, CacheEntry, cacheKey(), CacheOptions, clearApiCache(), fetchJson() (+8 more)

### Community 10 - "Google Classroom API"
Cohesion: 0.11
Nodes (15): ClassroomActivity, FakeClassroomService (test double), FakeCourses (test double), FakeDriveService (test double), FakeClassroomService, FakeCourses, FakeCourseWork, FakeDriveFiles (+7 more)

### Community 11 - "Google Drive Integration"
Cohesion: 0.12
Nodes (12): ClassroomCourse, drive_files_from_submission(), _due_label(), get_google_provider(), GoogleApiProvider, submission_links_from_submission(), SubmissionLink, grading_health() (+4 more)

### Community 12 - "UI History & Icons"
Cohesion: 0.11
Nodes (22): AppIcon, EmptyState(), SearchBox(), SkeletonRows(), GraderQueue(), ReferenceQueueCard(), referenceQueueStatus(), GraderWrap() (+14 more)

### Community 13 - "Database & Migrations"
Cohesion: 0.12
Nodes (19): Session, _ensure_activity_columns(), _ensure_cache_columns(), _ensure_columns(), _ensure_grading_ai_attempt_columns(), _ensure_grading_criterion_columns(), _ensure_grading_job_columns(), _ensure_grading_submission_columns() (+11 more)
...
Cohesion: 0.14
Nodes (24): datetime, Any, AccountProfile, _cache_hit(), _ttl(), _TtlCacheEntry, _bounded_repr(), _bounded_text() (+16 more)

### Community 18 - "Google Auth & Profile"
Cohesion: 0.10
Nodes (19): ConnectView(), InlineError(), DoneView(), HistoryView(), AppIcon(), IconName, icons, getInitials() (+11 more)

### Community 19 - "Grader Workflow UI"
Cohesion: 0.18
Nodes (8): Manual Classroom Posting (API limitation), GraderTopbar(), postingClipboardText(), scoreOf(), scoreOf, studentLabel, SummaryStat, GradingJob
...
Cohesion: 0.20
Nodes (14): get_google_provider(), make_google_provider(), Legacy single-user helper. Use make_google_provider() for multi-user flows., auth_me(), _build_export_read(), _contains_invalid_grant(), create_export(), google_auth_http_exception() (+6 more)
...
Cohesion: 0.19
Nodes (15): GradingEngineRequest, _build_messages(), parse_litellm_result(), LLM Model Catalog (local/remote/cached modes), Rubric Inference (description-first vs sample-based), GradingEngineResult, _request(), test_build_messages_cowrite_forbids_numeric_grade() (+7 more)

### Community 32 - "Test Cleanup & Token Store"
Cohesion: 0.22
Nodes (6): build_oauth_authorization_url(), clear_google_provider_caches(), auth_start(), Shared pytest configuration for the API test suite.  Two isolation concerns ar, _reset_global_state(), test_build_oauth_authorization_url_uses_configured_web_client()
...
Cohesion: 0.42
Nodes (15): GoogleProvider, GradingJob, GradingSubmission, PrivacyAuditRead, Session, SubmissionFile, PrivacyAudit, PrivacyAuditRow (+7 more)
...
Cohesion: 0.20
Nodes (8): GradingEngineRequest, LlmModelEntry, _build_rubric_messages(), LiteLlmGradingEngine, model_entry(), test_engine_calls_litellm_with_scrubbed_payload(), test_engine_falls_back_to_json_object_when_schema_unsupported(), test_engine_uses_json_schema_response_format_when_supported()

### Community 60 - "Community 60"
Cohesion: 0.21
Nodes (8): GradingReadiness, inspect_grading_readiness(), _missing_provider_keys(), MockGradingEngine, _probe_valid_key(), Offline check via litellm.validate_environment: which provider env vars     the, Live check via litellm.check_valid_key. Returns (ok, detail); ok is None     wh, Non-raising readiness report for the configured grading engine. Used by     the
...
## Knowledge Gaps
- **145 isolated node(s):** `schema_version`, `default_model`, `enabled`, `display_name`, `use_cases` (+140 more)
...
- **Why does `Student Pseudonymization before AI` connect `Grader Review Panel` to `Logging & Observability`, `Test Fixtures & Settings`, `Privacy & PII Protection`, `Grading Stream Tests`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Why does `Settings` connect `LLM Model Catalog` to `DeepSeek Model Config`, `Claude Code Settings`, `Test Fixtures & Settings`, `Community 63`?**
  _High betweenness centrality (0.091) - this node is a cross-community bridge._
- **Are the 71 inferred relationships involving `GradingStatus` (e.g. with `Activity` and `datetime`) actually correct?**
  _`GradingStatus` has 71 INFERRED edges - model-reasoned connections that need verification._
- **Are the 65 inferred relationships involving `GradingSubmission` (e.g. with `Activity` and `datetime`) actually correct?**
  _`GradingSubmission` has 65 INFERRED edges - model-reasoned connections that need verification._
- **Are the 55 inferred relationships involving `GoogleProvider` (e.g. with `Activity` and `datetime`) actually correct?**
  _`GoogleProvider` has 55 INFERRED edges - model-reasoned connections that need verification._
